module GuidancesHelper
  ...
  def planetary_navigation
    content_tag(:div, :class => 'button_daily_card temporal_navigation') do
      ...
      (@sequence == 7 ? ''.html_safe : link_to(source_cards_marketing_text('temporal_navigation', 'planetary', 'return'), planet_card_guidance_path(:birthday_id => @birthday.id, :planet => @planet, :sequence => 7, :personality => @personality, :zodiac => @zodiac), :remote => true, :onclick => "rotateTo(0);", :class => 'lunar_navigation', :data => {:turbolinks => false})) +
